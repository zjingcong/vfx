...
void PerlinNoise::init(void)
{
	...
	for (i = 0 ; i < myB ; i++) {
		...
		g1[i] = (float)((random() % (myB + myB)) - myB) / myB;

		for (j = 0 ; j < 2 ; j++)
			g2[i][j] = (float)((random() % (myB + myB)) - myB) / myB;
		normalize2(g2[i]);

		for (j = 0 ; j < 3 ; j++)
			g3[i][j] = (float)((random() % (myB + myB)) - myB) / myB;
		normalize3(g3[i]);
	}
...
const float PerlinNoiseGustavson::eval( const Vector& P ) const
{
